...
void calc_harm_value(_WarePoint ware)
{
	...
	for (i = 1; i < 20; i++) {   	//计算谐波总含有率及各次谐波含有率和谐波值
		if (abs(TempUa[1]) > 60) {
			HarmData.Ua[i] = (TempUa[i]*HarmGainCoef[i]/TempUa[1]*realdata.LineUa)/U_COEF;
		...
		if (abs(TempUb[1]) > 60) {
			HarmData.Ub[i] = (TempUb[i]*HarmGainCoef[i]/TempUb[1]*realdata.LineUb)/U_COEF;
		...
		if (abs(TempUc[1]) > 60) {
			HarmData.Uc[i] = (TempUc[i]*HarmGainCoef[i]/TempUc[1]*realdata.LineUc)/U_COEF;
		...
		if (abs(TempIa[1] > 10)) {
			if(attCoef.HarmICoef[0]!=0) {
				HarmData.Ia[i] = (TempIa[i]*HarmGainCoef[i]/TempIa[1]*realdata.LineIa)/I_COEF;
		...
		if (abs(TempIb[1] > 10)) {
			HarmData.Ib[i] = (TempIb[i]*HarmGainCoef[i]/TempIb[1]*realdata.LineIb)/I_COEF;
		...
		if (abs(TempIc[1] > 10)) {
			HarmData.Ic[i] = (TempIc[i]*HarmGainCoef[i]/TempIc[1]*realdata.LineIc)/I_COEF;
