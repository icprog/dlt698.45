...
INT32S Esam_WriteThenRead(INT32S fd, INT8U* Tbuf, INT16U Tlen, INT8U* Rbuf){
    ...
    if(sem_spi0_0!=NULL)
    	sem_wait(sem_spi0_0);
	for(index=0;index<6;index++)//只做6次循环
	{
 ...
 *************************************************************/
INT32S Esam_GetTermiSingleInfo(INT32S fd, INT8U type, INT8U* Rbuf) {   //&&已测
...
INT32S Esam_SymKeyUpdate(INT32S fd, SID_MAC SidMac,INT8U* Data2) {
	...
	len+=1;
	 ...
	 Result = Esam_WriteThenRead(fd, (INT8U*)GetInfo_ESAM,len,Rbuf);
