...
void getFRZtime(CLASS_6013 class6013, CLASS_6015 class6015, TS ts, INT8U *buf,
		INT16U seqsec) {
	int i = 0;
	INT16U daysec = 0;
 ...
 */
void CreateSaveHead(char *fname, CSD_ARRAYTYPE csds,
		HEADFIXED_INFO *taskhead_info, HEAD_UNIT *headoad_unit, INT32U seqnum,
		INT32U seqsec) {
	INT16U len_tmp = 0;
	int i = 0, j = 0;
	OAD oad_m;
	...
	for (i = 0; i < csds.num; i++) {
		...
		if (csds.csd[i].type == 0)	//OAD
				{
			if (csds.csd[i].csd.oad.OI == 0x202a
					|| csds.csd[i].csd.oad.OI == 0x6040
					|| csds.csd[i].csd.oad.OI == 0x6041
					|| csds.csd[i].csd.oad.OI == 0x6042) {
				fprintf(stderr, "OAD=%04x_%02x%02x 存在固定头，不计算长度\n",
						csds.csd[i].csd.oad.OI, csds.csd[i].csd.oad.attflg,
						csds.csd[i].csd.oad.attrindex);
				continue;
			}
			len_tmp = CalcOIDataLen(csds.csd[i].csd.oad);	//多一个数据类型
			taskhead_info->reclen += len_tmp;
			getOneUnit(&headoad_unit[taskhead_info->oadnum], oad_m,
					csds.csd[i].csd.oad, len_tmp);
			...
			for (j = 0; j < csds.csd[i].csd.road.num; j++) {
				...
				taskhead_info->reclen += len_tmp;
				getOneUnit(&headoad_unit[taskhead_info->oadnum],
						csds.csd[i].csd.road.oad, csds.csd[i].csd.road.oads[j],
						len_tmp);
				taskhead_info->oadnum++;
			}
		}
...
void CreateSaveEventHead(char *fname, ROAD eve_oad,
		HEADFIXED_INFO *taskhead_info, HEAD_UNIT *headoad_unit) {
	INT16U len_tmp = 0;
	int i = 0;
	FILE *fp = NULL;
	if (eve_oad.num == 0xee || eve_oad.num == 0) {
		asyslog(LOG_WARNING, "cjsave eve:csds.num=%d,return!!!error!!!",
				eve_oad.num);
		return;
	}
	...
	for (i = 0; i < eve_oad.num; i++) {
		if (eve_oad.oads[i].OI == 0xeeee)
			break;
		if (eve_oad.oads[i].OI == 0x2020 || eve_oad.oads[i].OI == 0x2022
				|| eve_oad.oads[i].OI == 0x201e)
			continue;
		len_tmp = CalcOIDataLen(eve_oad.oads[i]);	//多一个数据类型
		taskhead_info->reclen += len_tmp;
		getOneUnit(&headoad_unit[taskhead_info->oadnum], eve_oad.oad,
				eve_oad.oads[i], len_tmp);
		taskhead_info->oadnum++;
	}
	fp = fopen(fname, "w+");
	fwrite(taskhead_info, sizeof(HEADFIXED_INFO), 1, fp);
...
void fillRECdata(OADDATA_SAVE *OADdata, INT8U OADnum, INT8U *databuf,
		HEADFIXED_INFO taskhead_info, HEAD_UNIT *headoad_unit, TS OADts,
		CLASS_6013 class6013, CLASS_6015 class6015) {
	int i = 0, j = 0, mm = 0;
	INT16U indexn = 0;
...
//	pos6040 = 2*(1+sizeof(DateTimeBCD));//记录第三个位置为6041
	...
	if (databuf[0] == 0 && class6015.savetimeflag != 6)	//采集开始时间为空 6表示冻结时标取抄表上来的6042
			{
		databuf[0] = 0x1c;
		getFRZtime(class6013, class6015, OADts, &databuf[1],
	...
	for (i = 0; i < taskhead_info.oadnum; i++)	//开头三个时标
			{
		if (headoad_unit[i].oad_r.OI == 0x6040
				|| headoad_unit[i].oad_r.OI == 0x6041
				|| headoad_unit[i].oad_r.OI == 0x6042)
			continue;
		fprintf(stderr, "\n---------%d:0x%04x-0x%04x\n", i,
				headoad_unit[i].oad_m.OI, headoad_unit[i].oad_r.OI);
		for (j = 0; j < OADnum; j++) {
			fprintf(stderr, "\n%d:0x%04x-0x%04x\n", j, OADdata[j].oad_m.OI,
					OADdata[j].oad_r.OI);
			if (headoad_unit[i].oad_m.OI == OADdata[j].oad_m.OI
				...
				if (class6015.savetimeflag == 6
						&& headoad_unit[i].oad_r.OI == 0x2021)
...
void fillEVEdata(OADDATA_SAVE *OADdata, INT8U OADnum, INT8U *databuf,
		HEADFIXED_INFO taskhead_info, HEAD_UNIT *headoad_unit) {
	int i = 0, j = 0, mm = 0;
	INT16U index = 0;
	...
	for (i = 0; i < taskhead_info.oadnum; i++) {
		fprintf(stderr, "\n查找%d: 0x%04x-0x%04x\n", i, headoad_unit[i].oad_m.OI,
				headoad_unit[i].oad_r.OI);
		for (j = 0; j < OADnum; j++) {
			fprintf(stderr, "\n%d:0x%04x-0x%04x\n", i, OADdata[i].oad_m.OI,
					OADdata[i].oad_r.OI);
			if ((headoad_unit[i].oad_m.OI == OADdata[j].oad_m.OI/* ||
...
//	return runtimes;
//}
int SaveOADData(INT8U taskid, OAD oad_m, OAD oad_r, INT8U *databuf, int datalen,
		TS ts_res) {
	TSA tsa;
...
void saveREADOADdata(INT8U taskid, TSA tsa, OADDATA_SAVE *OADdata, INT8U OADnum,
		TS OADts) {
	pthread_mutex_lock(&mutex_savetask);
#if 1
	DbgPrintToFile1(1,
			"\n##############saveREADOADdata = %d　TSA=%02x %02x %02x %02x %02x %02x %02x %02x",
			OADnum, tsa.addr[0], tsa.addr[1], tsa.addr[2], tsa.addr[3],
			tsa.addr[4], tsa.addr[5], tsa.addr[6], tsa.addr[7]);
	fprintf(stderr, "\n##############saveREADOADdata = %d", OADnum);
	INT8U prtIndex = 0;
	for (prtIndex = 0; prtIndex < OADnum; prtIndex++) {
		fprintf(stderr, "\nOADdata[%d] oad_m = %04x oad_r =  %04x data[%d] =",
				prtIndex, OADdata[prtIndex].oad_m.OI,
				OADdata[prtIndex].oad_r.OI, OADdata[prtIndex].datalen);
		DbgPrintToFile1(1,
				"\nOADdata[%d] oad_m = %04x oad_r =  %04x data[%d] = %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
				prtIndex, OADdata[prtIndex].oad_m.OI,
				OADdata[prtIndex].oad_r.OI, OADdata[prtIndex].datalen,
				OADdata[prtIndex].data[0], OADdata[prtIndex].data[1],
				OADdata[prtIndex].data[2], OADdata[prtIndex].data[3],
				OADdata[prtIndex].data[4], OADdata[prtIndex].data[5],
				OADdata[prtIndex].data[6], OADdata[prtIndex].data[7],
				OADdata[prtIndex].data[8], OADdata[prtIndex].data[9],
				OADdata[prtIndex].data[10], OADdata[prtIndex].data[11]);
		INT8U prtIndex1 = 0;
		for (prtIndex1 = 0; prtIndex1 < OADdata[prtIndex].datalen;
				prtIndex1++) {
	...
	INT16U curseq = 0;
	INT32U blklen = 0, headlen = 0, reclen = 0, seqsec = 0, seqnum = 0, oadsec =
			0;
	int i = 0;
	long int savepos = 0;
	...
	fprintf(stderr, "\n文件存储时间%04d-%02d-%02d %02d:%02d:%02d  存储文件 %s\n",
			OADts.Year, OADts.Month, OADts.Day, OADts.Hour, OADts.Minute,
			OADts.Sec, fname);
	if (access(fname, F_OK) != 0) {
		fprintf(stderr, "\n创建文件头\n");
		CreateSaveHead(fname, class6015.csds, &taskhead_info, headoad_unit,
				seqnum, seqsec);	//写文件头信息并返回
	} else {
		...
		if (seqnum != taskhead_info.seqnum || seqsec != taskhead_info.seqsec
				|| oadnum + 3 != taskhead_info.oadnum
				|| reclen + 24 != taskhead_info.reclen)	//任务变更 todo 再加上oad不再这个方案里
						{
			asyslog(LOG_INFO,
					"删除原来任务%d文件，创建文件头 任务信息:%d %d %d %d 文件中信息:%d %d %d %d",
					taskid, seqnum, seqsec, oadnum + 3, reclen + 24,
					taskhead_info.seqnum, taskhead_info.seqsec,
					taskhead_info.oadnum, taskhead_info.reclen);
			fprintf(stderr, "\n删除原来任务，创建文件头\n");
	...
	fillRECdata(OADdata, OADnum, data_buf, taskhead_info, headoad_unit, OADts,
			class6013, class6015);	//把数据组好，放入缓存

	fprintf(stderr, "\nsavepos=%d data_buf(%d):", savepos,
			taskhead_info.reclen);
...
//事件存储
INT16U saveREADOADevent(ROAD eve_road, TSA tsa, OADDATA_SAVE *OADdata,
		INT8U OADnum, TS OADts) {
#if 1
	prtTSA(tsa);
	DbgPrintToFile1(1, "\n##############saveREADOADevent = %d OI = %04x",
			OADnum, eve_road.oad.OI);
	...
	if (fp == NULL)	//文件没内容 组文件头，如果文件已存在，提取文件头信息
	{
		...
		savepos = sizeof(HEADFIXED_INFO)
				+ taskhead_info.oadnum * sizeof(HEAD_UNIT);
		fir_save = 1;
		...
		while (!feof(fp))	//轮寻文件
		{
			...
			if (memcmp(&tsadata[1], tsa.addr, TSA_LEN) == 0
					&& tsadata[0] == 0x55)	//找到了存储结构的位置，一个存储结构可能含有unitnum个单元
							{
				savepos = ftell(fp);	//位置为跳过最开头的tsa的位置，即符合tsa的第一条记录的位置
				fprintf(stderr, "\n找到了tsa savepos=%d\n", savepos);
	...
	DbgPrintToFile1(1, "存储事件  OADnum=%d 存储信息:长度%d oad个数%d 存储个数%d 存储间隔%d",
			OADnum, taskhead_info.reclen, taskhead_info.oadnum,
			taskhead_info.seqnum, taskhead_info.seqsec);
...
//	fprintf(stderr,"\n");
	...
	fp = fopen(fname, "r+");	//格式
	fprintf(stderr, "\n存储savepos=%d len=%d\n", savepos,
			taskhead_info.reclen + TSA_LEN + 1);
	fseek(fp, savepos, SEEK_SET);
