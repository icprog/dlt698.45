...
extern INT8S use6013find6015or6017(INT8U cjType, INT16U fanganID,
		TI interval6013, CLASS_6015* st6015);
	...
	for (timePartIndex = 0; timePartIndex < str_runtime.num; timePartIndex++) {
		...
		if (min_start <= min_end) {
			if ((now_min > min_start) && (now_min < min_end)) {
				...
				return 1;
			} else if ((str_runtime.type == 2)
					&& ((now_min == min_end) || (now_min == min_start))) {
				return 1;
			}
		}
	}
...
INT8U filterInvalidTask(INT16U taskIndex) {
	...
	if (time_in_shiduan(list6013[taskIndex].basicInfo.runtime,
			list6013[taskIndex].basicInfo.interval) == 1)	//在抄表时段内
			{
		return 1;
	}
	return 0;
...
INT8U cmpTaskPrio(INT16U taskIndex1, INT16U taskIndex2) {
	...
	if (list6013[taskIndex1].basicInfo.runprio
		...
		return 1;
			...
			< list6013[taskIndex2].basicInfo.interval.units) {
		return 2;
	} else if (list6013[taskIndex1].basicInfo.cjtype
			> list6013[taskIndex2].basicInfo.cjtype) {
		return 1;
	} else if (list6013[taskIndex1].basicInfo.cjtype
			< list6013[taskIndex2].basicInfo.cjtype) {
		return 2;
...
//			,portOAD.OI,portOAD.attflg,portOAD.attrindex,port485);
	...
	if ((portOAD.attflg != 0x02) || (portOAD.attrindex != port485)) {
		return 0;
	}
	return 1;
}
	...
	for (tIndex = 0; tIndex < TASK6012_MAX; tIndex++) {
		if (readCoverClass(oi, tIndex, &class6013, sizeof(CLASS_6013),
			...
			if (class6013.cjtype != rept) {
				memcpy(&list6013[total_tasknum].basicInfo, &class6013,
						sizeof(CLASS_6013));

				TS taskStartTime;
				TimeBCDToTs(list6013[total_tasknum].basicInfo.startime,
						&taskStartTime);

				//加上延时
				if (list6013[total_tasknum].basicInfo.delay.interval > 0) {
					tminc(&taskStartTime,
							list6013[total_tasknum].basicInfo.delay.units,
							list6013[total_tasknum].basicInfo.delay.interval);
				}

				INT8U timeCmp = TScompare(ts_now, taskStartTime);
#if 1
				asyslog(LOG_NOTICE,
						"timeCmp = %d 当前时间 %04d-%02d-%02d %02d:%02d:%02d 任务开始时间 %04d-%02d-%02d %02d:%02d:%02d",
						timeCmp, ts_now.Year, ts_now.Month, ts_now.Day,
						ts_now.Hour, ts_now.Minute, ts_now.Sec,
						taskStartTime.Year, taskStartTime.Month,
						taskStartTime.Day, taskStartTime.Hour,
				...
				if (getZone("GW") != 0) {
					...
					if (readCoverClass(0x6015,
							list6013[total_tasknum].basicInfo.sernum, &st6015,
							sizeof(CLASS_6015), coll_para_save) == 1) {
						if (st6015.csds.csd[0].csd.road.oad.OI == 0x5004) {
							infoReplenish.unitReplenish[infoReplenish.tasknum++].taskID =
									list6013[total_tasknum].basicInfo.taskID;
						}
...
INT8S init4204Info() {
	...
	if (readCoverClass(0x4204, 0, &broadcase4204, sizeof(CLASS_4204),
			para_vari_save) == 1) {
		flagDay_4204[0] = 1;
		flagDay_4204[1] = 1;
	}
...
int proxy_dar_fill(PROXY_GETLIST *dest_list, PROXY_GETLIST get_list) {
	...
	case ProxyGetRequestList:
		for (i = 0; i < dest_list->num; i++) {
			...
			if (dest_list->proxy_obj.objs[i].dar != success) {
				...
				memcpy(&dest_list->data[index],
						&dest_list->proxy_obj.objs[i].tsa.addr[0], addrlen);
				index += addrlen;
				...
				for (j = 0; j < dest_list->proxy_obj.objs[i].num; j++) {
					result_num++;
					index += create_OAD(0, &dest_list->data[index],
							dest_list->proxy_obj.objs[i].oads[j]);
	...
	case ProxyGetRequestRecord:
		...
		if (dest_list->proxy_obj.record.dar != success) {
			...
			memcpy(&dest_list->data[index],
					&dest_list->proxy_obj.buf[proxy_index], addrlen);
			proxy_index += addrlen;
			index += addrlen;
			...
			//OAD
			memcpy(&dest_list->data[index],
					&dest_list->proxy_obj.buf[proxy_index], 4);
			index += 4;
			proxy_index += 4;
			rsd_len = get_BasicRSD(0, &dest_list->proxy_obj.buf[proxy_index],
					tmpBuf, &RSD_type);
			proxy_index += rsd_len;
			fprintf(stderr,
					"proxylen=%d,rsd_len=%d,addrlen=%d,proxy_index=%d\n",
					dest_list->proxylen, rsd_len, sizeof(oadtmp), proxy_index);
			rcsd_len = dest_list->proxylen - addrlen - rsd_len - sizeof(oadtmp);
			//RCSD
			memcpy(&dest_list->data[index],
					&dest_list->proxy_obj.buf[proxy_index], rcsd_len);
			index += rcsd_len;
			//CHOICE
	...
	case ProxyActionRequestList:
		for (i = 0; i < dest_list->num; i++) {
			...
			} else if (get_list.proxy_obj.doTsaList[i].dar
					== request_overtime) {
			...
			if (dest_list->proxy_obj.doTsaList[i].dar != success) {
				...
				memcpy(&dest_list->data[index],
						&dest_list->proxy_obj.doTsaList[i].tsa.addr[0],
						addrlen);
				index += addrlen;
				...
				for (j = 0; j < dest_list->proxy_obj.doTsaList[i].num; j++) {
					result_num++;
					index += create_OAD(0, &dest_list->data[index],
							dest_list->proxy_obj.doTsaList[i].setobjs[j].oad);
					dest_list->data[index++] =
							dest_list->proxy_obj.doTsaList[i].dar;
	...
	case ProxyActionThenGetRequestList:
		...
		for (i = 0; i < dest_list->num; i++) {
			...
			} else if (get_list.proxy_obj.doTsaThenGet[i].dar
					== request_overtime) {
			...
			if (dest_list->proxy_obj.doTsaThenGet[i].dar != success) {
				...
				memcpy(&dest_list->data[index],
						&dest_list->proxy_obj.doTsaThenGet[i].tsa.addr[0],
						addrlen);
				index += addrlen;
				result_index = index;	//记录SEQUENCE of 对象属性描述符及结果
				index++;
				...
				for (j = 0; j < dest_list->proxy_obj.doTsaThenGet[i].num; j++) {
					...
					//设置的对象属性描述符
					index +=
							create_OAD(0, &dest_list->data[index],
									dest_list->proxy_obj.doTsaThenGet[i].setoads[j].oad_set);
					//及其设置结果
					dest_list->data[index++] =
							dest_list->proxy_obj.doTsaThenGet[i].dar;
					...
					index +=
							create_OAD(0, &dest_list->data[index],
									dest_list->proxy_obj.doTsaThenGet[i].setoads[j].oad_get);
					//及其设置结果
...
//	memset(&proxyList_manager.proxy_obj.record,0,sizeof(GETRECORD));	//无TSA满足要求时要进行组帧，此处不能清0
	if (get6001ObjByTSA(proxy.strProxyList.proxy_obj.record.tsa, &obj6001)
			!= 1) {
...
//		dataindex += proxy_one_fill_record(proxy.strProxyList.proxy_obj.record, 0, NULL,0x21, &proxyList_manager.data[dataindex]);
		...
		if (obj6001.basicinfo.port.OI == PORT_485) {
			set_port_active(1, 1);
			set_port_active(2, 1);
			memcpy(&cjcommProxy.strProxyList.proxy_obj.record,
					&proxy.strProxyList.proxy_obj.record, sizeof(GETRECORD));
			cjcommProxy.strProxyList.proxytype = proxy.strProxyList.proxytype;
			cjcommProxy.strProxyList.proxylen = proxy.strProxyList.proxylen;
			cjcommProxy.isInUse = 3;
			proxyInUse.devUse.rs485Need = 1;
		} else if (obj6001.basicinfo.port.OI == PORT_ZB) {
			memcpy(&cjcommProxy_plc.strProxyList.proxy_obj.record,
					&proxy.strProxyList.proxy_obj.record, sizeof(GETRECORD));
			cjcommProxy_plc.strProxyList.proxytype =
					proxy.strProxyList.proxytype;
...
void Pre_ProxyDoThenGetRequestList(CJCOMM_PROXY proxy) {
	...
	for (i = 0; i < num; i++) {
		if (get6001ObjByTSA(proxy.strProxyList.proxy_obj.doTsaThenGet[i].tsa,
				&obj6001) != 1) {	//TSA未找到
...
//			dataindex += proxy_one_fill(proxy.strProxyList.proxy_obj.doTsaThenGet[i], 0, NULL,0x21, &proxyList_manager.data[dataindex]);
			...
			if (obj6001.basicinfo.port.OI == PORT_485) {
				memcpy(
						&cjcommProxy.strProxyList.proxy_obj.doTsaThenGet[num_485++],
						&proxy.strProxyList.proxy_obj.doTsaThenGet[i],
						sizeof(DO_Then_GET));
				cjcommProxy.strProxyList.num = num_485;

			} else if (obj6001.basicinfo.port.OI == PORT_ZB) {
				memcpy(
						&cjcommProxy_plc.strProxyList.proxy_obj.doTsaThenGet[num_zb++],
						&proxy.strProxyList.proxy_obj.doTsaThenGet[i],
						sizeof(DO_Then_GET));
				cjcommProxy_plc.strProxyList.num = num_zb;
			}
	...
	fprintf(stderr, "\n&&&&&&&&&&&&&&&&&&&&&&&&&&&");
	printProxyDoThenGet(cjcommProxy.strProxyList.num,
			cjcommProxy.strProxyList.proxy_obj.doTsaThenGet);
	proxyList_manager.datalen = dataindex;
	fprintf(stderr, "\n代理任务分配");
	if (num_485 > 0) {
		set_port_active(1, 1);
...
//			dataindex += proxy_one_fill(proxy.strProxyList.proxy_obj.doTsaList[i], 0, NULL,0x21, &proxyList_manager.data[dataindex]);
			...
			if (obj6001.basicinfo.port.OI == PORT_485) {
				memcpy(&cjcommProxy.strProxyList.proxy_obj.doTsaList[num_485++],
						&proxy.strProxyList.proxy_obj.doTsaList[i],
						sizeof(ACTION_SET_OBJ));
				cjcommProxy.strProxyList.num = num_485;
			} else if (obj6001.basicinfo.port.OI == PORT_ZB) {
				memcpy(
						&cjcommProxy_plc.strProxyList.proxy_obj.doTsaList[num_zb++],
						&proxy.strProxyList.proxy_obj.doTsaList[i],
						sizeof(ACTION_SET_OBJ));
				cjcommProxy_plc.strProxyList.num = num_zb;
			}
...
void Pre_F209TransAction(CJCOMM_PROXY proxy) {
	...
	cjcommProxy_plc.isInUse = 1;
	proxyInUse.devUse.plcNeed = 1;
	memcpy(&cjcommProxy_plc.strProxyList, &proxy.strProxyList,
...
//	OADtoBuff(proxy.strProxyList.transcmd.oad,proxyList_manager.data);
	dataindex += create_OAD(0, proxyList_manager.data,
			proxy.strProxyList.proxy_obj.transcmd.oad);
	proxyList_manager.datalen = dataindex;
	//COMDCB合法性判断
	...
	if (dar != success) {
		...
		if (oad == PORT_ZB) {
			...
			memcpy(&cjcommProxy.strProxyList, &proxy.strProxyList,
					sizeof(PROXY_GETLIST));
			cjcommProxy.isInUse = 3;
			proxyInUse.devUse.rs485Need = 1;
		}
...
void divProxy(CJCOMM_PROXY proxy) {
	...
	case ProxyActionThenGetRequestList:
		printProxyDoThenGet(proxyList_manager.num,
				proxyList_manager.proxy_obj.doTsaThenGet);
...
//	fprintf(stderr," ret=%d,cmd=%d,pid=%d\n",ret,mq_h.cmd,mq_h.pid);
	if (ret > 0) {
		...
		if (mq_h.pid == cjdeal) {
			...
			DEBUG_TIME_LINE("\n收到代理召测\n");
			memcpy(&cjcommProxy_Tmp.strProxyList, rev_485_buf,
					sizeof(PROXY_GETLIST));
			fprintf(stderr, "proxy.strProxyList.num=%d  len=%d\n",
					cjcommProxy_Tmp.strProxyList.num,
					cjcommProxy_Tmp.strProxyList.proxylen);
...
//			cjcommProxy_Tmp.strProxyList.datalen=0;		//清除代理返回数据
			memset(&cjcommProxy_Tmp.strProxyList.data, 0,
					sizeof(cjcommProxy_Tmp.strProxyList.data));
				...
				for (findIndex = 0; findIndex < total_tasknum; findIndex++) {
					...
					if (list6013[findIndex].basicInfo.taskID
						...
						INT8S ret = mqs_send((INT8S *) TASKID_485_2_MQ_NAME,
								...
								sizeof(INT16S));
						ret = mqs_send((INT8S *) TASKID_485_1_MQ_NAME, cjdeal,
								1, OAD_PORT_485_1, (INT8U *) &findIndex,
								sizeof(INT16S));
					}
				}
			}
...
//			fprintf(stderr,"\n---------------------------------------datalen = %d",cjcommProxy_plc.strProxyList.datalen);
			...
			if (cjcommProxy_plc.strProxyList.datalen < 512) {
				memcpy(&proxyList_manager.data[index],
						cjcommProxy_plc.strProxyList.data,
						cjcommProxy_plc.strProxyList.datalen);
				proxyList_manager.datalen +=
						cjcommProxy_plc.strProxyList.datalen;
				proxy_dar_fill(&proxyList_manager,
						cjcommProxy_plc.strProxyList);
				proxyInUse.devUse.plcReady = 1;
				proxyList_manager.proxy_obj.f209Trans.dar =
	...
	if (proxyInUse.devUse.rs485Need == 1) {
		if (proxyInUse.devUse.rs485_1_Active == 0
				&& proxyInUse.devUse.rs485_2_Active == 0)
...
//	fprintf(stderr,"\ndispatch_thread start \n");
	...
	while (1) {
		...
		if (para_change485[0] || (para_change485[1] && isNeed4852)) {
			fprintf(stderr,
					"para_change485[0] = %d para_change485[1] = %d isNeed4852=%d 参数变更等待 485线程处理无效线程",
		...
		if (tastIndex > -1) {
			...
			//计算下一次抄读此任务的时间;
			list6013[tastIndex].ts_next = calcnexttime(
					list6013[tastIndex].basicInfo.interval,
					list6013[tastIndex].basicInfo.startime,
					list6013[tastIndex].basicInfo.delay);

			INT8S ret = mqs_send((INT8S *) TASKID_485_2_MQ_NAME, cjdeal, 1,
					OAD_PORT_485_2, (INT8U *) &tastIndex, sizeof(INT16S));
			fprintf(stderr, "\n 向485 2线程发送任务ID = %d \n", ret);
			ret = mqs_send((INT8S *) TASKID_485_1_MQ_NAME, cjdeal, 1,
					OAD_PORT_485_1, (INT8U *) &tastIndex, sizeof(INT16S));
			fprintf(stderr, "\n 向485 1线程发送任务ID = %d \n", ret);
			ret = mqs_send((INT8S *) TASKID_plc_MQ_NAME, cjdeal, 1, OAD_PORT_ZB,
					(INT8U *) &tastIndex, sizeof(INT16S));

			//任务时间到，判断是否为交采和统计数据需要冻结
			terminalTaskFreeze(list6013[tastIndex].basicInfo.taskID,
					list6013[tastIndex].basicInfo.sernum);
...
void printinfoReplenish(INT8U flag) {
	...
	for (prtIndex = 0; prtIndex < infoReplenish.tasknum; prtIndex++) {
		DbgPrintToFile1(3, "第%d个需要补抄任务ID = %d－－－－－－－－－－\n", prtIndex,
				infoReplenish.unitReplenish[prtIndex].taskID);
		...
		if (readParaClass(oi, targetMeter, fileIndex) == 1) {
			if (targetMeter->sernum != 0 && targetMeter->sernum != 0xffff) {
				...
				fprintf(stderr,
						...
						targetMeter->basicinfo.addr.addr[0],
						targetMeter->basicinfo.addr.addr[1],
						targetMeter->basicinfo.addr.addr[2],
						targetMeter->basicinfo.addr.addr[3],
						targetMeter->basicinfo.addr.addr[4],
						targetMeter->basicinfo.addr.addr[5],
						targetMeter->basicinfo.addr.addr[6],
						targetMeter->basicinfo.addr.addr[7]);
				if (memcmp(addr.addr, targetMeter->basicinfo.addr.addr,
						(addr.addr[0] + 1)) == 0)	//一致性测试PROXY_02
						{
					isMeterExist = 1;
					ret = 1;
					break;
...
void dispatchTask_proccess() {
	...
	mqd_485_main = mmq_open((INT8S *) PROXY_485_MQ_NAME, &attr_485_main,
			O_RDONLY);
...
int main(int argc, char *argv[]) {
	...
	asyslog(LOG_INFO, "进程 %s PID = %d", JProgramInfo->Projects[1].ProjectName,
			JProgramInfo->Projects[1].ProjectID);
