...
//温度及补偿值对应公式
static INT16S AllGainVal[25][2] = { { 80, -180 }, { 75, -165 }, { 70, -150 }, {
		65, -130 }, { 60, -110 }, { 55, -90 }, { 50, -70 },
		{ 45, -50 },	//80'C,75'C,70'C,65'C,60'C,55'C,50'C,45'C
		{ 40, -30 }, { 35, -10 }, { 30, 0 }, { 25, 0 }, { 20, 0 }, { 15, 8 }, {
				10, 20 },
		{ 5, 37 },	//40'C,35'C,30'C,25'C,20'C,15'C,10'C,5'C
		{ 0, 42 }, { -5, 47 }, { -10, 52 }, { -15, 57 }, { -20, 62 },
		{ -25, 67 }, { -30, 72 }, { -35, 77 }, { -40, 82 }//0'C,-5'C,-10'C,-15'C,-20'C,-25'C,-30'C,-35'C
};

//高温Ua,Ub,Uc,Ia,Ib,Ic补偿系数
static FP32 HighTempCoef[6][2] = { { 30, 1.000 }, { 30, 1.000 }, { 30, 1.000 },
		{ 30, 1.000 }, { 30, 1.000 }, { 30, 1.000 } };
//低温Ua,Ub,Uc,Ia,Ib,Ic补偿系数
static FP32 LowTempCoef[6][2] = { { -60, 1.000 }, { -60, 1.000 },
		{ -60, 1.000 }, { -60, 1.000 }, { -60, 1.000 }, { -60, 1.000 } };
...
void read_tempcoef_cfg() {
	...
	if (fp != NULL) {
		for (i = 0; i < 6; i++) {
			...
			if (strcmp(type, "Ua") == 0)
				no = 0;
			else if (strcmp(type, "Ub") == 0)
				no = 1;
			else if (strcmp(type, "Uc") == 0)
				no = 2;
			else if (strcmp(type, "Ia") == 0)
				no = 3;
			else if (strcmp(type, "Ib") == 0)
				no = 4;
			else if (strcmp(type, "Ic") == 0)
				no = 5;
			if (hcoef == 0) {
				hcoef = 1.0;
				fprintf(stderr, "高温补偿系数文件设置0，修改为1");
			}
			if (lcoef == 0) {
				lcoef = 1.0;
				fprintf(stderr, "低温补偿系数文件设置0，修改为1");
			}
...
#define PQCoef		756.01035      //三相四
	...
	case U:					//电压
		...
		if (tread <= 10 * U_COEF)
			tread = 0;		//特殊处理，当电压<10V,进行零飘处理，清除0
...
//	realdata.St = realdata.Sa + realdata.Sb + realdata.Sc;		//trans_regist(S,2,RRec[r_St],tmm);
	...
	if (realdata.Temp >= HighTempCoef[0][0])
		realdata.Ua = realdata.Ua * HighTempCoef[0][1];
	else if (realdata.Temp <= LowTempCoef[0][0])
		realdata.Ua = realdata.Ua * LowTempCoef[0][1];
	if (realdata.Temp >= HighTempCoef[1][0])
		realdata.Ub = realdata.Ub * HighTempCoef[1][1];
	else if (realdata.Temp <= LowTempCoef[1][0])
		realdata.Ub = realdata.Ub * LowTempCoef[1][1];
	if (realdata.Temp >= HighTempCoef[2][0])
		realdata.Uc = realdata.Uc * HighTempCoef[2][1];
	else if (realdata.Temp <= LowTempCoef[2][0])
		realdata.Uc = realdata.Uc * LowTempCoef[2][1];
	if (realdata.Temp >= HighTempCoef[3][0])
		realdata.Ia = realdata.Ia * HighTempCoef[3][1];
	else if (realdata.Temp <= LowTempCoef[3][0])
		realdata.Ia = realdata.Ia * LowTempCoef[3][1];
	if (realdata.Temp >= HighTempCoef[4][0])
		realdata.Ib = realdata.Ib * HighTempCoef[4][1];
	else if (realdata.Temp <= LowTempCoef[4][0])
		realdata.Ib = realdata.Ib * LowTempCoef[4][1];
	if (realdata.Temp >= HighTempCoef[5][0])
		realdata.Ic = realdata.Ic * HighTempCoef[5][1];
	else if (realdata.Temp <= LowTempCoef[5][0])
		realdata.Ic = realdata.Ic * LowTempCoef[5][1];
...
//	fprintf(stderr,"IJ=%d-%d-%d  ",IaJ,IbJ,IcJ);
	...
	//采样到的负数转换到正值
	if (IaJ < 0)
		realdata.Pga = IaJ + 360 * PHASE_COEF;
	else
		realdata.Pga = IaJ;
	if (IbJ < 0)
		realdata.Pgb = IbJ + 360 * PHASE_COEF;
	else
		realdata.Pgb = IbJ;
	if (IcJ < 0)
		realdata.Pgc = IcJ + 360 * PHASE_COEF;
	else
		realdata.Pgc = IcJ;
 ...
 * 输入参数：ts：当前时间
 * */
void calc_minute_ave_u(TS ts, INT32U ua, INT32U ub, INT32U uc, INT32U *avgua,
		INT32U *avgub, INT32U *avguc) {
	static TS lastts;
...
INT8U get_rates_no(TS ts_t, CLASS_4016 class4016) {
	...
	if (changed != JProgramInfo->oi_changed.oi4016) {
		...
		readCoverClass(0x4016, 0, &class4016, sizeof(CLASS_4016),
				para_vari_save);
		oldmin = 0xff;
	}
	if (ts_t.Minute == oldmin && oldmin != 0xff)
		return rate;
	oldmin = ts_t.Minute;
	if (class4016.day_num > MAX_PERIOD_RATE)
		return rate;
...
//		shmm_getpubdata()->ac_energy.PosPt_Rate[nRate]=es->PosPt_Rate[nRate];
	...
	dbg_prt("总：PosPt=%d, NegPt=%d, PosQt=%d, NegQt=%d\n", es->PosPt_All,
			es->NegPt_All, es->PosQt_All, es->NegQt_All);
	dbg_prt("正向有功:	Pa=%d, Pb=%d, Pc=%d\n\r", es->PosPa_All, es->PosPb_All,
			es->PosPc_All);
	dbg_prt("反向有功:	Pa=%d, Pb=%d, Pc=%d\n\r", es->NegPa_All, es->NegPb_All,
			es->NegPc_All);
	dbg_prt("正向无功:	Qa=%d, Qb=%d, Qc=%d\n\r", es->PosQa_All, es->PosQb_All,
			es->PosQc_All);
	dbg_prt("反向无功:	Qa=%d, Qb=%d, Qc=%d\n\r", es->NegQa_All, es->NegQb_All,
			es->NegQc_All);
}
 ...
 */
void sum_reactive_energy(TS nowts, INT8U nRate, INT32U PFlag, INT8U QWord1,
		INT8U QWord2, INT32U EngPosQt, INT32U EngNegQt, ACEnergy_Sum *es) {
	INT8U flag[8], i;
	...
	dbg_prt("Q1=%d,Q2=%d,Q3=%d,Q4=%d\n", es->Q1_Qt_All, es->Q2_Qt_All,
			es->Q3_Qt_All, es->Q4_Qt_All);
	//无功组合特征字1：正向无功
...
void read_engergy_regist(TS ts, INT32S fp) {
	...
	sum_reactive_energy(ts, nRate, realdata.PFlag, QFeatureWord1, QFeatureWord2,
			energycurr.PosQt, energycurr.NegQt, &energysum);
}
...
void InitACSCoef() {
	...
	for (i = 0; i < 3; i++) {
		if (attCoef.HarmUCoef[i] == 0)
			attCoef.HarmUCoef[i] = 1;
		if (attCoef.HarmICoef[i] == 0)
...
void InitACSEnergy() {
	...
	readret = readCoverClass(0, 0, &energysum, sizeof(ACEnergy_Sum),
			acs_energy_save);
	if (readret == 1) {		//读取文件成功
...
void realdataprint(TS nowts) {
	...
	if (oldsec == nowts.Sec)
		return;
	oldsec = nowts.Sec;
	dbg_prt("电压:	UA=%.1f, UB=%.1f, UC=%.1f", (float)realdata.Ua/U_COEF,
			(float)realdata.Ub/U_COEF, (float)realdata.Uc/U_COEF);
	dbg_prt("电流:	IA=%.3f, IB=%.3f, IC=%.3f, IL=%.3f", (float)realdata.Ia/I_COEF,
			(float)realdata.Ib/I_COEF, (float)realdata.Ic/I_COEF,
			(float)realdata.I0/I_COEF);
	dbg_prt("有功:	PT=%.1f, PA=%.1f, PB=%.1f, PC=%.1f", (float)realdata.Pt/P_COEF,
			(float)realdata.Pa/10, (float)realdata.Pb/P_COEF,
			(float)realdata.Pc/10);
	dbg_prt("无功:	QZ=%.1f, QA=%.1f, QB=%.1f, QC=%.1f", (float)realdata.Qt/Q_COEF,
			(float)realdata.Qa/Q_COEF, (float)realdata.Qb/Q_COEF,
			(float)realdata.Qc/Q_COEF);
	dbg_prt("视在:	SZ=%.1f, SA=%.1f, SB=%.1f, SC=%.1f", (float)realdata.St/S_COEF,
			(float)realdata.Sa/S_COEF, (float)realdata.Sb/S_COEF,
			(float)realdata.Sc/S_COEF);
	...
	dbg_prt("相角:	Pga=%.1f, Pgb=%.1f, Pgc=%.1f", (float)realdata.Pga/PHASE_COEF,
			(float)realdata.Pgb/PHASE_COEF, (float)realdata.Pgc/PHASE_COEF);
	...
	dbg_prt("1min平均电压:	UA=%.1f, UB=%.1f, UC=%.1f", (float)realdata.AvgUa/U_COEF,
			(float)realdata.AvgUb/U_COEF, (float)realdata.AvgUc/U_COEF);
	dbg_prt("相序标识:SFlag=%02x", realdata.SFlag);
	dbg_prt("温度:Temp=%d", realdata.Temp);
	dbg_prt("基波电压:	UA=%.1f, UB=%.1f, UC=%.1f", (float)realdata.LineUa/U_COEF,
			(float)realdata.LineUb/U_COEF, (float)realdata.LineUc/U_COEF);
	dbg_prt("基波电流:	IA=%.3f, IB=%.3f, IC=%.3f", (float)realdata.LineIa/I_COEF,
			(float)realdata.LineIb/I_COEF, (float)realdata.LineIc/I_COEF);
	dbg_prt("\n\r");
}
/*
...
void check_reg_print(INT32S device_id) {
	if (device_id == ATT_VER_ID) {
		dbg_prt("\n\r 大电流相角 校表系数");
		dbg_prt("PhaseA=%02x_%02x_%02x", attCoef.PhaseA0[0], attCoef.PhaseA0[1],
				attCoef.PhaseA0[2]);
		dbg_prt("PhaseB=%02x_%02x_%02x", attCoef.PhaseB0[0], attCoef.PhaseB0[1],
				attCoef.PhaseB0[2]);
		dbg_prt("PhaseC=%02x_%02x_%02x", attCoef.PhaseC0[0], attCoef.PhaseC0[1],
				attCoef.PhaseC0[2]);
	} else {
		dbg_prt("\n\r 相角 校表系数\n");
		dbg_prt("PhaseA=%02x_%02x_%02x", attCoef.PhaseA[0], attCoef.PhaseA[1],
				attCoef.PhaseA[2]);
		dbg_prt("PhaseB=%02x_%02x_%02x", attCoef.PhaseB[0], attCoef.PhaseB[1],
				attCoef.PhaseB[2]);
		dbg_prt("PhaseC=%02x_%02x_%02x", attCoef.PhaseC[0], attCoef.PhaseC[1],
				attCoef.PhaseC[2]);
		dbg_prt("\n\r 小电流相角 校表系数");
		dbg_prt("PhaseA=%02x_%02x_%02x", attCoef.PhaseA1[0], attCoef.PhaseA1[1],
				attCoef.PhaseA1[2]);
		dbg_prt("PhaseB=%02x_%02x_%02x", attCoef.PhaseB1[0], attCoef.PhaseB1[1],
				attCoef.PhaseB1[2]);
		dbg_prt("PhaseC=%02x_%02x_%02x", attCoef.PhaseC1[0], attCoef.PhaseC1[1],
				attCoef.PhaseC1[2]);
		dbg_prt("\n\r 电压 校表系数");
		dbg_prt("UA=%02x_%02x_%02x", attCoef.UA[0], attCoef.UA[1],
				attCoef.UA[2]);
		dbg_prt("UB=%02x_%02x_%02x", attCoef.UB[0], attCoef.UB[1],
				attCoef.UB[2]);
		dbg_prt("UC=%02x_%02x_%02x", attCoef.UC[0], attCoef.UC[1],
				attCoef.UC[2]);
		dbg_prt("\n\r 电流 校表系数");
		dbg_prt("IA=%02x_%02x_%02x", attCoef.IA[0], attCoef.IA[1],
				attCoef.IA[2]);
		dbg_prt("IB=%02x_%02x_%02x", attCoef.IB[0], attCoef.IB[1],
				attCoef.IB[2]);
		dbg_prt("IC=%02x_%02x_%02x", attCoef.IC[0], attCoef.IC[1],
				attCoef.IC[2]);
		dbg_prt("\n\r 功率 校表系数");
		dbg_prt("PA=%02x_%02x_%02x", attCoef.PA[0], attCoef.PA[1],
				attCoef.PA[2]);
		dbg_prt("PB=%02x_%02x_%02x", attCoef.PB[0], attCoef.PB[1],
				attCoef.PB[2]);
		dbg_prt("PC=%02x_%02x_%02x", attCoef.PC[0], attCoef.PC[1],
				attCoef.PC[2]);
		dbg_prt("\n\r 谐波电压系数");
		dbg_prt("Ua=%d, Ub=%d, Uc=%d", attCoef.HarmUCoef[0],
				attCoef.HarmUCoef[1], attCoef.HarmUCoef[2]);
		dbg_prt("\n\r 谐波电流系数");
		dbg_prt("Ia=%d, Ib=%d, Ic=%d", attCoef.HarmICoef[0],
				attCoef.HarmICoef[1], attCoef.HarmICoef[2]);
	}
}

void energysum_print(ACEnergy_Sum energysum_tmp) {
	fprintf(stderr, "总电能示值：PosPt=%d, NegPt=%d, PosQt=%d, NegQt=%d\n",
			energysum_tmp.PosPt_All, energysum_tmp.NegPt_All,
			energysum_tmp.PosQt_All, energysum_tmp.NegQt_All);
	dbg_prt("总电能示值(4舍5入)：PosPt=%d, NegPt=%d, PosQt=%d, NegQt=%d\n",
			energysum.PosPt_All, energysum.NegPt_All, energysum.PosQt_All,
			energysum.NegQt_All);
	dbg_prt("正向有功:(4舍5入)	Pa=%d, Pb=%d, Pc=%d\n", energysum.PosPa_All,
			energysum.PosPb_All, energysum.PosPc_All);
	dbg_prt("正向有功:	Pa=%d, Pb=%d, Pc=%d\n", energysum_tmp.PosPa_All,
			energysum_tmp.PosPb_All, energysum_tmp.PosPc_All);
	dbg_prt("反向有功:	Pa=%d, Pb=%d, Pc=%d\n", energysum_tmp.NegPa_All,
			energysum_tmp.NegPb_All, energysum_tmp.NegPc_All);
	dbg_prt("正向无功:	Qa=%d, Qb=%d, Qc=%d\n", energysum_tmp.PosQa_All,
			energysum_tmp.PosQb_All, energysum_tmp.PosQc_All);
	dbg_prt("反向无功:	Qa=%d, Qb=%d, Qc=%d\n", energysum_tmp.NegQa_All,
			energysum_tmp.NegQb_All, energysum_tmp.NegQc_All);
	dbg_prt("正向有功费率: P1=%d, P2=%d, P3=%d, P4=%d\n", energysum_tmp.PosPt_Rate[0],
			energysum_tmp.PosPt_Rate[1], energysum_tmp.PosPt_Rate[2],
			energysum_tmp.PosPt_Rate[3]);
	...
	dbg_prt("反向无功费率:	Q1=%d, Q2=%d, Q3=%d, Q4=%d\n\r", energysum.NegQt_Rate[0],
			energysum.NegQt_Rate[1], energysum.NegQt_Rate[2],
			energysum.NegQt_Rate[3]);
	dbg_prt("象限1：总及四费率1:Qt=%d,Q1=%d,Q2=%d,Q3=%d,Q4=%d\n",
			energysum_tmp.Q1_Qt_All, energysum_tmp.Q1_Qt_Rate[0],
			energysum_tmp.Q1_Qt_Rate[1], energysum_tmp.Q1_Qt_Rate[2],
			energysum_tmp.Q1_Qt_Rate[3]);
	dbg_prt("象限2：总及四费率2:Qt=%d,Q1=%d,Q2=%d,Q3=%d,Q4=%d\n",
			energysum_tmp.Q2_Qt_All, energysum_tmp.Q2_Qt_Rate[0],
			energysum_tmp.Q2_Qt_Rate[1], energysum_tmp.Q2_Qt_Rate[2],
			energysum_tmp.Q2_Qt_Rate[3]);
	dbg_prt("象限3：总及四费率3:Qt=%d,Q1=%d,Q2=%d,Q3=%d,Q4=%d\n",
			energysum_tmp.Q3_Qt_All, energysum_tmp.Q3_Qt_Rate[0],
			energysum_tmp.Q3_Qt_Rate[1], energysum_tmp.Q3_Qt_Rate[2],
			energysum_tmp.Q3_Qt_Rate[3]);
	dbg_prt("象限4：总及四费率4:Qt=%d,Q1=%d,Q2=%d,Q3=%d,Q4=%d\n",
			energysum_tmp.Q4_Qt_All, energysum_tmp.Q4_Qt_Rate[0],
			energysum_tmp.Q4_Qt_Rate[1], energysum_tmp.Q4_Qt_Rate[2],
			energysum_tmp.Q4_Qt_Rate[3]);
}
...
//		spifp = OpenACS_I();
		write_coef_reg(spifp, &chksum, &chksum1);
		syslog(LOG_NOTICE,
				"ChkSum(%x) old(%x) ChkSum1(%x) old1(%x) have change,rewrite_coef_reg\n",
				chksum, oldchksum, chksum1, oldchksum1);
//		fprintf(stderr,"ChkSum(%d) old(%d) ChkSum1(%d) old1(%d) have change,rewrite_coef_reg\n",chksum,oldchksum,chksum1,oldchksum1);
		oldchksum = chksum;
		oldchksum1 = chksum1;
	}
	calc_minute_ave_u(nowts, realdata.Ua, realdata.Ub, realdata.Uc,
			&realdata.AvgUa, &realdata.AvgUb, &realdata.AvgUc);
	//暂时去掉谐波处理过程。
			...
			if (bettery_getV(&bett[0], &bett[1]) == TRUE) {
				...
				if ((bett[1] >= MIN_BATTWORK_VOL) && saveflag != 1) {//掉电后且电池电量满足，只保存一次
					...
					if (bett[1] < MIN_BATTWORK_VOL)
						syslog(LOG_NOTICE,
								"底板电源已关闭，电池电压=%f V, saveflag=%d 不保存电量！！！",
								bett[1], saveflag);
				}
			}
		}
...
//	fprintf(stderr,"计量芯片版本：%06X, 接线方式=%X(0600:三相四，1200：三相三)\n",JProgramInfo->dev_info.ac_chip_type,JProgramInfo->dev_info.WireType);
	asyslog(LOG_NOTICE, "计量芯片版本：%06X, 接线方式=%X(0600:三相四，1200：三相三)\n",
			...
			JProgramInfo->dev_info.WireType);
}
