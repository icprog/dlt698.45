	...
	{{level1,"2.参数定值", 	NULL, 				MENU_NOPASSWD},		NULL},
...
#ifdef HUBEI
		{{level2,"7.配置参数", 		NULL, 				MENU_NOPASSWD},		NULL},
#else
		...
		{{level2,"8.配置参数", 		NULL, 				MENU_NOPASSWD},		NULL},
#endif
			{{level3,"1.终端编号", 	NULL, 					MENU_NOPASSWD},		NULL},
				{{level4,"1.十进制", 		menu_jzqzddr10, 			MENU_NOPASSWD},		NULL},
				{{level4,"2.十六进制",	menu_jzqzddr16, 			MENU_NOPASSWD},		NULL},
#ifdef HUBEI
			{{level3,"2.通讯参数设置", 	NULL, 	MENU_ISPASSWD_EDITMODE},		NULL},
#else
			{{level3,"2.通信通道", 	NULL, 	MENU_ISPASSWD_EDITMODE},		NULL},
#endif
				{{level4,"1.通信方式", 	menu_settx, 				MENU_NOPASSWD},		NULL},
				{{level4,"2.短信中心", 	menu_jzqtelephone, 			MENU_NOPASSWD},		NULL},
				{{level4,"3.通信参数", 	menu_masterapn, 			MENU_NOPASSWD},		NULL},
				{{level4,"4.以太网参数", 	menu_eth0para, 				MENU_NOPASSWD},		NULL},
#ifdef HUBEI
				{{level4,"5.虚拟专网参数", 	menu_Virpara, 				MENU_NOPASSWD},		NULL},
#endif
			{{level3,"3.终端复位", 	NULL, 						MENU_ISPASSWD},		NULL},
				...
				{{level4,"3.参数初始化", 	menu_initjzqpara, 	MENU_ISPASSWD},		NULL},
			...
			{{level3,"6.现场调试", 	NULL, 				MENU_NOPASSWD},		NULL},
				...
				{{level4,"7.配置设置",menu_TorF_info,		MENU_NOPASSWD},		NULL},
#ifdef SHANDONG
				{{level4,"8.485被抄",menu_485func_change,		MENU_NOPASSWD},		NULL},
#endif
#ifdef HUBEI
			{{level3,"7.电表参数设置", 		NULL, 	MENU_NOPASSWD},		NULL},
				{{level4,"1.修改测量点", 		menu_fksetmeter,	MENU_ISPASSWD},	NULL},
				{{level4,"2.添加测量点", 		menu_fkaddmeter,	MENU_ISPASSWD},	NULL},
				{{level4,"3.删除测量点", 		menu_fkdelmeter,	MENU_ISPASSWD},	NULL},
#endif
	{{level1,"3.控制状态", 	menu_control, 				MENU_NOPASSWD},		NULL},
#ifdef HUBEI
	{{level1,"4.电能表示数", 	NULL, 				MENU_NOPASSWD},		NULL},
		{{level2,"1.正向有功电量", 	menu_realdata_ZJ, 						MENU_NOPASSWD},		NULL},
		{{level2,"2.三相电压", 	menu_realdata_DY, 						MENU_NOPASSWD},		NULL},
		{{level2,"3.三相电流", 	menu_realdata_DL, 						MENU_NOPASSWD},		NULL},
		{{level2,"4.一象限无功", 	menu_realdata_X1, 						MENU_NOPASSWD},		NULL},
		{{level2,"5.二象限无功", 	menu_realdata_X2, 						MENU_NOPASSWD},		NULL},
		{{level2,"6.三象限无功", 	menu_realdata_X3, 						MENU_NOPASSWD},		NULL},
		{{level2,"7.四象限无功", 	menu_realdata_X4, 						MENU_NOPASSWD},		NULL},
		{{level2,"8.ABC相有功", 	menu_realdata_P1, 						MENU_NOPASSWD},		NULL},
		{{level2,"9.ABC相无功", 	menu_realdata_P2, 						MENU_NOPASSWD},		NULL},
		{{level2,"10.其他", 	menu_realdata_OT, 						MENU_NOPASSWD},		NULL},
#else
...
#ifdef HUBEI
	{{level1,"8.终端管理与维护", 	NULL, 			MENU_NOPASSWD},		NULL},
		{{level2,"1.终端初始化", 	NULL, 						MENU_ISPASSWD},		NULL},
			...
			{{level3,"4.恢复出厂设置", 	menu_initjzqparaf3, 	MENU_ISPASSWD},		NULL},
		{{level2,"2.终端登录状态", 	menu_login_stat, 			MENU_NOPASSWD},		NULL},
		{{level2,"3.手动抄表", 	NULL, 				MENU_NOPASSWD},		NULL},
...
void menu_control(){
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		menu_control_showstate((char*)"保电", shmm_getdevstat()->ctrpar.Baodian.Valid&0x01, pos);
		pos.y += FONTSIZE*3;
...
//		dbg_prt("para error\n");
	...
	gui_setpos(&pos, rect_Client.left, rect_Client.top+3*FONTSIZE-2);
	gui_textshow((char*)"当前正向有功电量示数", pos, LCD_NOREV);
	sprintf((char*)str,"总% 10.2f %s",shmm_getpubdata()->ac_energy.PosPt_All*1.0/(ENERGY_CONST*ENERGY_COEF), surfix);
	gui_setpos(&pos, rect_Client.left+4*FONTSIZE, rect_Client.top+5*FONTSIZE);
	gui_textshow((char*)str, pos, LCD_NOREV);
	memset(str, 0, 100);
...
//		dbg_prt("\npara error");
	...
	sprintf((char*)str,"正向无功总 % 8.2f %s",shmm_getpubdata()->ac_energy.PosQt_All*1.0/(ENERGY_CONST*ENERGY_COEF), "kVarh");
	pos.x = rect_Client.left;
	pos.y += 2*FONTSIZE+offset_y;
	gui_textshow((char*)str, pos, LCD_NOREV);
	memset(str, 0, 100);
...
void fk_realV_showDY(LcdDataItem *item,int itemNum,INT8U cldno,INT8U *surfix)//当前电表电压显示
{
	int offset_y = 3;
		...
		memset(str, 0, 100);
//		gui_setpos(&pos, rect_Client.left, rect_Client.top+3*FONTSIZE-2);
//		gui_textshow((char*)"当前测量点电压", pos, LCD_NOREV);
...
void fk_realC_showDL(LcdDataItem *item,int itemNum,INT8U cldno,INT8U *surfix)//当前电表电压显示
{
	...
	memset(str, 0, 100);
//	gui_setpos(&pos, rect_Client.left, rect_Client.top+3*FONTSIZE-2);
//	gui_textshow((char*)"当前测量点电流", pos, LCD_NOREV);
...
//				dbg_prt(" %d", arr_did[i]);
			...
			{
			case 1:
				if(ParaAll->f10.para_mp[cldno-1].Port == PORT_JC)
				{
				...
				else
				{
					if(ParaAll->f10.para_mp[cldno-1].Port == PORT_ZB)
						mqcount = requestdata_485_ZB_Block(ParaAll->f10.para_mp[cldno-1].MPNo, (INT8U*)PLC_REV_GUI_MQ,5, 3117,item);
...
void show_msg(char *text, Point text_pos){
	...
	len = strlen(text);
	//dbg_prt("\n text=%s len=%d", text, len);
	memset(str, 0, CHAENUM_PERLINE);
	while(i<=len){
		//dbg_prt("\n text[%d]=%02x  str=[%s] index_str=%d", i, text[i], str, index_str);
		if(text[i]>0x80){
...
void menu_zhongwen(){
	...
	//shmm_getdevstat()->controls.f32_cfg
	memcpy(str_neirong, shmm_getdevstat()->controls.f32_cfg.data, shmm_getdevstat()->controls.f32_cfg.len);//没有主站的中文信息数据
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		if(PressKey==ESC)
			break;
		gui_setpos(&pos, rect_Client.left+10*FONTSIZE, rect_Client.top+2*FONTSIZE);
		...
		memset(str, 0, 200);
		if(str[0]!=0)
			sprintf(str, "种类: %s  编号: %d", shmm_getdevstat()->controls.f32_cfg.type?"重要信息":"普通信息",
		...
		pos.y += FONTSIZE*3;
		memset(str, 0, 200);
		//show_msg("集中器和负控表是一个液晶程序，重要信息：普通信息。", pos);
		if(str_neirong[0]!=0)
...
//读共享内存获取购电量
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		if(PressKey!=NOKEY || first_flg==0){
			...
			memset(str, 0, 100);
			sprintf(str, "购电单号:%d", ParaAll->f47[zj_index-1].Electricity_purchase_order_number);
			gui_textshow(str, pos, LCD_NOREV);
			memset(str, 0, 100);
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		if(PressKey!=NOKEY || first_flg==0){
			...
			if(zj_index<=MAXNUM_SUMGROUP)
				...
				mpno = ParaAll->f11.f11[zj_index-MAXNUM_SUMGROUP].MPNo;
...
//					dbg_prt("\n 脉冲所待的测量点号超过8个 error!!!");
			...
			memset(str, 0, 100);
			if(zj_index<=MAXNUM_SUMGROUP)
				sprintf((char*)str,"有功功率:% 10.2f kW",shmm_getpubdata()->data_calc_by1min[zj_index-1].RPS);
			else{
#ifdef SPTF_III
				if(mpno!=0)
					sprintf((char*)str,"有功功率:% 10d kW",
						shmm_getpubdata()->pulse_calc_by_vstate.dd_realdata[mpno].P);
				else
			...
			memset(str, 0, 100);
			if(zj_index<=MAXNUM_SUMGROUP)
				sprintf((char*)str,"无功功率:% 10.2f kVar",shmm_getpubdata()->data_calc_by1min[zj_index-1].PQS);
			else{
#ifdef SPTF_III
				if(mpno!=0)
...
void menu_realE(){
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){//如果未处于轮显状态
		...
		if(PressKey!=NOKEY || first_flg==0){//如果有按键或者
			...
			switch(zf_index){
			case 1:
				fprintf(stderr,"总电能量 = %f\n",shmm_getpubdata()->data_calc_by1min[zj_index-1].DESP);
				...
				break;
			case 2:
				arr_data[0] = shmm_getpubdata()->data_calc_by1min[zj_index-1].DESQ;
				...
				break;
			case 3:
				arr_data[0] = shmm_getpubdata()->data_calc_by1min[zj_index-1].MESP;
				...
				break;
			case 4:
				arr_data[0] = shmm_getpubdata()->data_calc_by1min[zj_index-1].MESQ;
...
void show_load_page(INT8U *filename, int zj_index, int point_begin, int point_num)
{
	...
	for(i=point_begin-1; i<point_begin-1 +point_num; i++){
		...
		pos.y += 2*FONTSIZE + 3;
		//总加组有功功率曲线数据
		if(read_filedata_curve((char*)filename, zj_index, 1272, (i*15)/60, (i*15)%60, (void*)&curvedata)==1){
...
//	dbg_prt("\n menu_loadcurve filename=%s", filename);
	PressKey = NOKEY;
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		switch(PressKey)
		{
...
int get_erc(int ercno, ERC *erc, int erc_num){
	...
	for(i=shmm_getdevstat()->erc.EC1; i>=0; i--){
	...
	fprintf(stderr,"一般事件计数器值 = %d\n",shmm_getdevstat()->erc.EC2);
...
//	//ceshi
//	erc_count = 3;
//	erc[0].Buff[0]=5;erc[0].Err5.P[0]=1;
//	erc[1].Buff[0]=5;erc[1].Err5.P[0]=2;
//	erc[2].Buff[0]=5;erc[2].Err5.P[0]=4;
	PressKey = NOKEY;
	while(g_LcdPoll_Flag==LCD_NOTPOLL){//如果未处于轮显状态
		switch(PressKey)
		{
...
//	cldno = gui_GetJCMP();
//	if(cldno==0){
//		msgbox_label((char *)"无交采测量点", CTRL_BUTTON_OK);
//		return;
//	}
...
void menu_shiduanpara(){
	...
	initshiduan(ParaAll->f18.Period_PowerCtrl, sd_ts);
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		if(PressKey!=NOKEY || first_flg==0){
			...
			memset(str, 0, 100);
			sprintf(str, "时段控投入轮次:");
			getctrlround(ParaAll->f45[zj_index-1].Power_Round, str);
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 3*FONTSIZE;
			memset(str, 0, 100);
...
void menu_changxiupara(){
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		if(PressKey!=NOKEY || first_flg==0){
			...
			for(i=0;i<8;i++){
				if(ParaAll->f42[zj_index-1].Week_INTLimited_power & (1<<i)){
				//	strcat(str,"");
			...
			pos.x = rect_Client.left;
			pos.y += 2*FONTSIZE+3;
			lcd_A19toDate((char*)ParaAll->f42[zj_index-1].Time,&hour,&minute);
...
void menu_baotingpara(){
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		if(PressKey!=NOKEY || first_flg==0){
			...
			memset(str, 0, 100);
			pos.y += 3*FONTSIZE;
			lcd_A20toDate((char*)ParaAll->f44[zj_index-1].Start_Time_Stop,&year[0],&month[0],&day[0]);
			sprintf(str,"起始时间：%02d年%02d月%02d日", year[0],month[0],day[0]);
			gui_textshow(str, pos, LCD_NOREV);
			memset(str, 0, 100);
			pos.y += 3*FONTSIZE;
			lcd_A20toDate((char*)ParaAll->f44[zj_index-1].Start_Time_Stop,&year[1],&month[1],&day[1]);
			sprintf(str,"结束时间：%02d年%02d月%02d日", year[1],month[1],day[1]);
			gui_textshow(str, pos, LCD_NOREV);
			memset(str, 0, 100);
			pos.y += 3*FONTSIZE;
			sprintf(str,"定   值:% 8.3f kW", lcd_A02toDouble_decbits(ParaAll->f44[zj_index-1].Power_Stop,ParaAll->f44[zj_index-1].DecBits));
			gui_textshow(str, pos, LCD_NOREV);
		}
		PressKey = NOKEY;
		delay(300);
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		if(PressKey!=NOKEY || first_flg==0){
			...
			sprintf(str, "第一轮告警时间: %02d min",
				shmm_getdevstat()->ctrpar.Xiafukong[zj_index-1].AlarmTime_1.para_time);
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 2*FONTSIZE+2;
			memset(str, 0, 100);
			sprintf(str, "第二轮告警时间: %02d min",
					shmm_getdevstat()->ctrpar.Xiafukong[zj_index-1].AlarmTime_2.para_time);
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 2*FONTSIZE+2;
			memset(str, 0, 100);
			sprintf(str, "滑差时间: %02d min",
					shmm_getdevstat()->ctrpar.Xiafukong[zj_index-1].HuachaTime);
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 2*FONTSIZE+2;
			memset(str, 0, 100);
			sprintf(str, "延时时间: %02d min",
					shmm_getdevstat()->ctrpar.Xiafukong[zj_index-1].DongjeDelaytime.para_time);
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 2*FONTSIZE+2;
			memset(str, 0, 100);
			sprintf(str, "控制时间: %02d min",shmm_getdevstat()->ctrpar.Xiafukong[zj_index-1].DoDelaytime.para_time);
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 2*FONTSIZE+2;
			memset(str, 0, 100);
			fixvalue=lcd_A04tochar(shmm_getdevstat()->ctrpar.Xiafukong[zj_index-1].Xishu, &ctrl_flg);
...
void menu_yuedianpara(){
	...
	while(g_LcdPoll_Flag==LCD_NOTPOLL){
		...
		if(PressKey!=NOKEY || first_flg==0){
			...
			memset(str, 0, 100);
			sprintf(str, "月电控投入轮次:");
			getctrlround(ParaAll->f48[zj_index-1].ElectricityCtrl_Round_Flag, str);
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 3*FONTSIZE;
			memset(str, 0, 100);
			//fprintf(stderr,"电量定值：%d\n",ParaAll->f46[zj_index-1].Month_ElectricityCtrl);
			fix_value = ParaAll->f46[zj_index-1].Month_ElectricityCtrl;
			sprintf(str, "电量定值: % 8d %s", fix_value, suffix?"MWh":"kWh");
			gui_textshow(str, pos, LCD_NOREV);
			pos.y += 3*FONTSIZE;
			memset(str, 0, 100);
			//floatctrl_fixvalue=lcd_A04tochar(ParaAll->f20.Ratio_MonEECtrl, &floating_ctrl_flg);
			sprintf(str, "浮动系数:  %s % 3.2d%%", ParaAll->f20.Float_Fall?"下浮":"上浮",
					ParaAll->f20.Ratio_MonEECtrl);
			gui_textshow(str, pos, LCD_NOREV);
		}
		PressKey = NOKEY;
		delay(300);
...
void show_kvki(){
	...
	for(i=0;i<MAXNUM_SUMGROUP;i++){
		...
		sprintf(str, "    %d      %04d  %04d",i+1,
				...
				ParaAll->f25s.f25[i].MulPow_ISensor);
		gui_textshow(str, pos, LCD_NOREV);
	}
}
void show_kp(){
	...
	for(i=0;i< ROADNUM_PULSE;i++){
		...
		pos.y += 3*FONTSIZE;
		sprintf(str, "%d       %04d", i, ParaAll->f11.f11[i].MeterConstant);
		gui_textshow(str, pos, LCD_NOREV);
	}
#endif
}
