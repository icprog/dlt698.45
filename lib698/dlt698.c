...
extern int getRequestNormal(OAD oad, INT8U *data, CSINFO *csinfo,
		INT8U *sendbuf);
...
extern int doReponse(int server, int reponse, CSINFO *csinfo, int datalen,
		INT8U *data, INT8U *buf);
extern INT16U setRequestNormal(INT8U *data, OAD oad, INT8U *DAR, CSINFO *csinfo,
		INT8U *buf);
...
extern int Proxy_GetRequestlist(INT8U *data, CSINFO *csinfo, INT8U *sendbuf,
		INT8U piid);
extern int Proxy_GetRequestRecord(INT8U *data, CSINFO *csinfo, INT8U *sendbuf,
		INT8U piid);
extern int Proxy_TransCommandRequest(INT8U *data, CSINFO *csinfo,
		INT8U *sendbuf, INT8U piid);
extern int Proxy_DoRequestList(INT8U *data, CSINFO *csinfo, INT8U *sendbuf,
		INT8U piid, INT8U type);
extern int Proxy_DoThenGetRequestList(INT8U *data, CSINFO *csinfo,
		INT8U *sendbuf, INT8U piid, INT8U type);
...
int CheckHead(unsigned char* buf, CSINFO *csinfo) {
	...
	if (buf[0] == 0x68 && csinfo != NULL) {
		...
		DEBUG_TIME_LINE("direction: %s",
				ctl.ctl.dir?"send by server":"send by client");
		DEBUG_TIME_LINE("prm: %s",
...
int Link_Response(INT8U *apdu) {
	...
	linkresponse.request_time.year = ((linkresponse.request_time.year << 8)
			& 0xff00) | ((linkresponse.request_time.year >> 8) & 0x00ff);
	linkresponse.response_time.year = ((linkresponse.response_time.year << 8)
			& 0xff00) | ((linkresponse.response_time.year >> 8) & 0x00ff);
	linkresponse.reached_time.year = ((linkresponse.reached_time.year << 8)
			& 0xff00) | ((linkresponse.reached_time.year >> 8) & 0x00ff);
	fprintf(stderr, "\nLinkResponse: \nPIID = %02x \nResult = %02x ",
			linkresponse.piid.data, linkresponse.result);
	fprintf(stderr, "\nrequest time: %d-%d-%d %d:%d:%d",
			linkresponse.request_time.year, linkresponse.request_time.month,
			linkresponse.request_time.day_of_month,
			linkresponse.request_time.hour, linkresponse.request_time.minute,
			linkresponse.request_time.second);
	fprintf(stderr, "\nresponse time: %d-%d-%d  %d:%d:%d",
			linkresponse.response_time.year, linkresponse.response_time.month,
			linkresponse.response_time.day_of_month,
			linkresponse.response_time.hour, linkresponse.response_time.minute,
			linkresponse.response_time.second);
	fprintf(stderr, "\nreached time: %d-%d-%d  %d:%d:%d",
			linkresponse.reached_time.year, linkresponse.reached_time.month,
			linkresponse.reached_time.day_of_month,
			linkresponse.reached_time.hour, linkresponse.reached_time.minute,
			linkresponse.reached_time.second);
...
void GetconnetRequest(CONNECT_Request *request, INT8U *apdu) {
	...
	index = index + 2;
	memcpy((INT8U*) &request->ProtocolConformance, &apdu[index],
			sizeof(request->ProtocolConformance));
	index += sizeof(request->ProtocolConformance);
	memcpy((INT8U*) &request->FunctionConformance, &apdu[index],
			sizeof(request->FunctionConformance));
	...
	index += long_unsigned((INT8U *) &request->client_deal_maxApdu,
			&apdu[index]);
	request->expect_connect_timeout = (apdu[index] << 24)
			+ (apdu[index + 1] << 16) + (apdu[index + 2] << 8)
			+ (apdu[index + 3]);
	index = index + 4;
	...
	default: //数字签名
		bytenum = apdu[index];
		if (bytenum <= 40) {
			memcpy((INT8U*) &request->info.sigsecur.encrypted_code2,
					&apdu[index], bytenum + 1); //长度拷贝到目的缓冲区第0个字节
			index = index + bytenum + 1;
			bytenum = apdu[index];
			if (bytenum < 70)
				memcpy((INT8U*) &request->info.sigsecur.signature, &apdu[index],
						bytenum + 1);
		}
		break;
	}
...
void varconsult(CONNECT_Response *response, CONNECT_Request *request,
		CONNECT_Response *myvar) {
	int i = 0;

	//具体参照宣贯资料P10
	//商定的应用层协议版本号: 为服务器支持的协议版本号
	memcpy(&response->server_factory_version, &myvar->server_factory_version,
			sizeof(FactoryVersion));
	//商定的协议一致性块：客户机请求的协议一致性块与服务器支持的协议一致性块按位与的结果
	for (i = 0; i < 8; i++)
		response->ProtocolConformance[i] = myvar->ProtocolConformance[i]
				& request->ProtocolConformance[i];
	//商定的功能一致性块：客户机请求的功能一致性块与服务器支持的功能一致性块按位与的结果
	for (i = 0; i < 16; i++)
		response->FunctionConformance[i] = myvar->FunctionConformance[i]
				& request->FunctionConformance[i];
	//商定的应用层协议版本号:为服务器支持的协议版本号
	memcpy(&response->app_version, &request->expect_app_ver,
			sizeof(request->expect_app_ver));
...
int appConnectResponse(INT8U *apdu, CSINFO *csinfo, INT8U *buf) {
	...
	if (request.connecttype == 3) {
		ret = secureConnectRequest(&request.info.sigsecur,
...
int doSetAttribute(INT8U *apdu, CSINFO *csinfo, INT8U *buf) {
	...
	case SET_REQUEST_NORMAL:
		...
		doReponse(SET_RESPONSE, SET_REQUEST_NORMAL, csinfo, index, TmpDataBuf,
				buf);
		if (DAR == 0) { //sucess
...
//	}
	...
	case ProxySetRequestList:
		fprintf(stderr, "\n====ProxySetRequestList======\n");
		Proxy_DoRequestList(data, csinfo, sendbuf, piid_g.data,
				ProxySetRequestList);
		break;
	case ProxySetThenGetRequestList:
		fprintf(stderr, "\n====ProxySetThenGetRequestList======\n");
		Proxy_DoThenGetRequestList(data, csinfo, sendbuf, piid_g.data,
				ProxySetThenGetRequestList);
		break;
...
//				}
		...
		doReponse(ACTION_RESPONSE, ActionResponseNormalList, csinfo, index,
				TmpDataBuf, buf);
		break;
...
//				}
			...
			if (get_delay >= 0 && get_delay <= 5)
				sleep(get_delay);
			fprintf(stderr, "read oad=%04x_%02x%02x,get_delay=%d\n", oad.OI,
					oad.attflg, oad.attrindex, get_delay);
			//A-ReusltNormal.OAD
			index += create_OAD(0, &TmpDataBuf[index], oad);
		...
		doReponse(ACTION_RESPONSE, ActionThenGetResponseNormalList, csinfo,
				index, TmpDataBuf, buf);
		break;
	}
	return 1;
}
...
INT16S fillGetRequestAPDU(INT8U* sendBuf, CLASS_6015 obj6015, INT8U requestType) {
	...
	INT8U csdIndex = 0;
	INT8U len = 0;
	if ((requestType == GET_REQUEST_NORMAL_LIST)
...
//				len = OADtoBuff(obj6015.csds.csd[csdIndex].csd.oad,&sendBuf[length]);
				len = create_OAD(0, &sendBuf[length],
						obj6015.csds.csd[csdIndex].csd.oad);
				length += len;
			} else {
				fprintf(stderr,
						"fillGetRequestAPDU not OAD obj6015.sernum = %d,obj6015.cjtype = %d",
...
//				len = OADtoBuff(obj6015.csds.csd[csdIndex].csd.road.oad,&sendBuf[length]);
				len = create_OAD(0, &sendBuf[length],
						obj6015.csds.csd[csdIndex].csd.road.oad);
				length += len;
				/*采集上N次数据*/
				...
				if (obj6015.cjtype == TYPE_INTERVAL) {
					...
					//开始时间结束时间
					memcpy(&sendBuf[length],
							&obj6015.data.data[CURVE_INFO_STARTINDEX], 16);
					length += 16;
					//时间间隔
				...
				INT8U oadsIndex;
				for (oadsIndex = 0;
						oadsIndex < obj6015.csds.csd[csdIndex].csd.road.num;
						oadsIndex++) {

					sendBuf[length++] = 0; //OAD
//					len = OADtoBuff(obj6015.csds.csd[csdIndex].csd.road.oads[oadsIndex],&sendBuf[length]);
					len =
							create_OAD(0, &sendBuf[length],
									obj6015.csds.csd[csdIndex].csd.road.oads[oadsIndex]);
					length += len;
				}

			} else {
				fprintf(stderr,
						"fillGetRequestAPDU not ROAD obj6015.sernum = %d,obj6015.cjtype = %d",
						obj6015.sernum, obj6015.cjtype);
			}
...
INT16S composeProtocol698_SetActionThenGetRequest(INT8U* sendBuf, INT8U type,
		DO_Then_GET dogetOBJ) {
	...
	fprintf(stderr,
			...
			dogetOBJ.tsa.addr[0], dogetOBJ.tsa.addr[1], dogetOBJ.tsa.addr[2],
			dogetOBJ.tsa.addr[3], dogetOBJ.tsa.addr[4], dogetOBJ.tsa.addr[5],
			dogetOBJ.tsa.addr[6], dogetOBJ.tsa.addr[7], dogetOBJ.tsa.addr[8]);
...
//	sendLen += 4;
	...
	for (oadIndex = 0; oadIndex < dogetOBJ.num; oadIndex++) {
		sendLen += create_OAD(0, &sendBuf[sendLen],
				dogetOBJ.setoads[oadIndex].oad_set);
		INT16U dataIndex = 0;
		for (dataIndex = 0; dataIndex < dogetOBJ.setoads[oadIndex].len;
				dataIndex++) {
			sendBuf[sendLen++] = dogetOBJ.setoads[oadIndex].data[dataIndex];
		}
		sendLen += create_OAD(0, &sendBuf[sendLen],
				dogetOBJ.setoads[oadIndex].oad_get);
...
INT16S composeProtocol698_SetActionRequest(INT8U* sendBuf, INT8U type,
	...
	fprintf(stderr,
			" \n\n composeProtocol698_GetRequest  meterAddr : %02x  %02x  %02x%02x%02x%02x%02x%02x%02x\n\n",
			setOBJ.tsa.addr[0], setOBJ.tsa.addr[1], setOBJ.tsa.addr[2],
			setOBJ.tsa.addr[3], setOBJ.tsa.addr[4], setOBJ.tsa.addr[5],
			setOBJ.tsa.addr[6], setOBJ.tsa.addr[7], setOBJ.tsa.addr[8]);
...
//	sendLen += 4;
	for (oadIndex = 0; oadIndex < setOBJ.num; oadIndex++) {
		sendLen += create_OAD(0, &sendBuf[sendLen],
				setOBJ.setobjs[oadIndex].oad);
		INT16U dataIndex = 0;
		for (dataIndex = 0; dataIndex < setOBJ.setobjs[oadIndex].len;
				dataIndex++) {
...
INT16S composeProtocol698_GetRequest(INT8U* sendBuf, CLASS_6015 obj6015,
		TSA meterAddr) {
	INT8U PIID = 0x02;
...
INT16U composeProtocol698_GetRequestRecord(PROXY_GETLIST *getlist,
	...
	sendbuf[sendLen++] = PIID;
	memcpy(&sendbuf[sendLen], &getlist->proxy_obj.buf[datalen + 1],
			getlist->proxylen - datalen - 1);
 ...
 * */
//解析抄表回复报文
INT8U analyzeProtocol698(INT8U* Rcvbuf, INT8U* resultCount, INT16S recvLen,
 ...
 */
INT8U dealClientRequest(INT8U *apdu, CSINFO *csinfo, TimeTag timetag,
		INT8U *sendbuf) {
	INT16S SecurityRe = 0;
