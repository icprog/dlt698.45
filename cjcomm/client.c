...
MASTER_STATION_INFO getNextIpPort(void) {
    ...
    snprintf(&res.ip, sizeof(res.ip), "%d.%d.%d.%d", IpPool[index].ip[1], IpPool[index].ip[2], IpPool[index].ip[3], IpPool[index].ip[4]);
...
int RegularClient(struct aeEventLoop* ep, long long id, void* clientData) {
    ...
    if (nst->phy_connect_fd <= 0) {
        ...
        nst->phy_connect_fd = anetTcpConnect(errmsg, ip_port.ip, ip_port.port);
        if (nst->phy_connect_fd > 0) {
